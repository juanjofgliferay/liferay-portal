...
test.describe('DataSet Item Actions', () => {
	test('Setup environment', async ({dataSetManagerPage}) => {
	...
	test.describe('DataSet Fields Tab', () => {
		test('Dataset Fields Tab is selected', async ({
			...
			await test.step('Open Data Set Fields Tab', async () => {
				...
				await expect(
					page.getByRole('heading', {name: 'Fields'})
				).toBeVisible();
				await expect(
					page.getByText('No fields added yet.')
				).toBeVisible();
			});
		});
		...
		test('Can select Fields for the Dataset', async ({
			...
			const fieldModal =
				await test.step('Open Data Set Fields Modal', async () => {
					...
					await expect(
						page.getByRole('heading', {name: 'Add Fields'})
					).toBeVisible();
	...
	test.describe('DataSet Actions Tab', () => {
		...
		test('Can define an Item Action of type Link for the Dataset', async ({
			...
			await test.step('Open Data Set Item Action Form', async () => {
				...
				await expect(
					page.getByRole('heading', {name: 'New Item Action'})
				).toBeVisible();
			});

			await test.step('Fill Item Action Form', async () => {
				await page
					.getByPlaceholder('Action Name')
					.fill('Link Item Action');

				await page
					.getByLabel('TypeRequired', {exact: true})
					.selectOption({label: 'Link'});

				await page
					.getByLabel('URLRequired', {exact: true})
			...
			await test.step('Action is defined', async () => {
				await expect(
					page.locator('.orderable-table-row').nth(0)
				).toContainText('Link Item Action');
			});
		});
	});
		...
		test('Add DataSet fragment', async ({applicationsMenuPage, page}) => {
			...
			await test.step('Select Data Set View', async () => {
				...
				await page
					...
					.locator('li')
					.filter({hasText: 'Data Set View Test'})
					.first()
					.click();
				await page
					.frameLocator('iframe[title="Select"]')
	...
	test('Clean up environment', async ({dataSetManagerPage}) => {
		await dataSetManagerPage.deleteDataSet({
			name: 'New Data Set Item Actions',
		});
	});
});
